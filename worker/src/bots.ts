 ...
 * │                                                                            │
 * │ 지금 화면에 보이는 시각은 그게 아니다 — **자리 잡은 시각 + 모델 시간**이고,   │
 * │ 모델이 7~22초를 쓴다(실측). 0.85 로 두면 38초에 물어보고 45초 창 밖에서 답이  │
 * │ 오는데, 그건 다음 주제가 이미 떠 있는 화면이라 동문서답이거나(경계 넘김)      │
 * │ 남은 시간 클램프에 걸려 아예 안 물어본다(upgradeSpeech) — **창의 뒤쪽 절반이  │
 * │ 통째로 침묵**이 된다. 사용자가 본 게 그거다.                                 │
 * │                                                                            │
 * │ 그래서 **물어보는 창을 앞으로 당긴다.** 0.9~20초에 물어보면 답은 8~42초에     │
 * │ 나가서 창 전체에 고르게 퍼진다 — 두 상자가 지키려던 "분포가 신호가 되면 안    │
 * │ 된다"는 오히려 이쪽이 낫다. 바뀐 건 규칙이 아니라 **지연의 정체**다.          │
 * │                                                                            │
 * │ ★ 모델을 바꾸면(§15-1) 이 값도 같이 봐야 한다. 지연이 1초로 줄면 답이 창의     │
 * │   앞쪽 절반에만 몰린다 — 그때는 도로 0.85 쪽으로 넓히는 게 맞다.              │
 * └──────────────────────────────────────────────────────────────────────────┘
 */
const TOPIC_SPEAK_MIN_FRAC = 0.02;
const TOPIC_SPEAK_MAX_FRAC = 0.45;
