...
/**
 * **물리적으로 불가능한** 예산 (ms). 이 아래는 부르지 않는다.
 *
 * ┌─ 아래 2026-08-07 상자와 무엇이 다른가 ────────────────────────────────────┐
 * │ 그때 뺀 건 **10초 문턱**이었고, 뺀 이유는 예산이 짧아도 답이 가끔 들어와서다. │
 * │ 그 관찰은 지금도 맞다 — 실측 성공 왕복이 2.9 · 3.8 · 4.4 · 4.5 · 5.3초라     │
 * │ 10초 문턱은 그걸 전부 죽였다. 되살리지 않는다.                               │
 * │                                                                            │
 * │ 여기는 그 지대가 아니라 **답이 나올 수 없는 지대**다. 오리진은 컷을           │
 * │ `max(1000, 예산 - 3500)` 으로 잡으므로(app/api/internal/world-agent 의       │
 * │ AGENT_ROUNDTRIP_HEADROOM_MS), 예산이 4.5초 이하면 컷이 **1초로 바닥**을 친다. │
 * │ 이 저장소가 지금까지 기록한 제일 빠른 왕복이 2.9초다 — 1초 컷은 가끔도 안     │
 * │ 들어온다. 실제로 운영 로그에 `시간 초과 (1000ms)` 로 남는다 (2026-08-09,      │
 * │ 18건 중 3건).                                                              │
 * │                                                                            │
 * │ 그래서 이 문턱은 **발화를 줄이지 않는다.** 줄이는 건 버릴 게 확정된 호출뿐이고, │
 * │ 그 호출도 공급자 쿼터는 똑같이 먹는다 — 무응답(TTFT)이 바로 그 쿼터에서 온다. │
 * └────────────────────────────────────────────────────────────────────────────┘
 */
const DEAD_AGENT_BUDGET_MS = 4_600;
...
export class RoomDO {
  ...
  private async upgradeSpeech(
    ...
    // 오리진이 컷을 1초로 깎을 예산이면 부르지 않는다 (아래 상자).
    if (wait < DEAD_AGENT_BUDGET_MS) return;
