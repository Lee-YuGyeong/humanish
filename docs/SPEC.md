...
> - **`score`는 EXP의 원천이라 비울 수 없다.** 점수 규칙을 지우면 위 표의 경험치·레벨이 통째로 멈춘다. 그래서 **승패에서 유도한다 — 이긴 판 3, 진 판 1.** 진 판에도 1을 주는 건 참가 자체를 세기 위해서고, 안 그러면 계속 지는 사람의 레벨이 영영 0에 머문다.
...
| §15-2-결정 (전적) | `won`을 점수에서 유추하고, EXP는 그 점수 | `won`은 **진영 승패를 그대로** 적는다. 점수가 사라지므로 **EXP는 승패에서 유도한다 — 이긴 판 3, 진 판 1.** 안 정하면 레벨이 멈춘다 |
