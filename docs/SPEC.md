...
- **팀 승패 판정이 없다** — `/api/reveal`은 점수만 준다. `mostSuspectedHuman`은 미구현이고 아무 데서도 안 쓴다 (§8). 전적(§15-2-결정)은 이걸 기다리지 않고 **각자 자기 목표를 이뤘는지**(점수가 붙었는지)를 `won`으로 적는다 — 없는 판정을 화면에서 지어내면 결과 화면과 전적이 서로 다른 말을 하기 때문이다. 팀 승패를 정하면 그때 `match_results`에 컬럼을 더한다
