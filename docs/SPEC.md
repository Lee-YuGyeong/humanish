...
3~8인 채팅방에서 누가 AI인지 찾는 웹 게임. 정원은 방을 만들 때 정한다(기본 5). 빈자리는 AI 봇이 채우며 그 사실은 공개되지 않는다. 인간 중 한 명은 AI인 척해야 하는 스파이다.
...
**사람 2명이 들어온 정원 5인 방이면 5표 중 3표가 봇 표이고, 그 봇 표는 무작위다.** 지금 봇은 자기 아닌 사람 중 하나를 그냥 고른다 (`on_enter_phase`의 vote 훅). 그래서 점수가 실력보다 운에 좌우된다. **정원이 커질수록 나빠진다** — 정원이 방마다 달라진 뒤로는 최악이 8표 중 6표다 (§17.6).
...
| **15-3** | 봇을 채우는 시점 | lobby에서 미리 채우는가, 시작 버튼을 누른 순간 채우는가. 전자는 인원수로 봇 존재가 추론될 수 있다. **배치 규칙은 §17.4에서 정했다** — 시점만 남았다. **정원이 방마다 달라지면서 이 추론이 더 쉬워졌다** — 정원을 아는 사람은 "정원 − 아는 사람 수"로 봇 수를 바로 뺄 수 있다. 반대로 정원을 안 내려보내면 좌석 칸을 못 그린다. 결론은 내지 않는다 | §13-1 |
...
**정원이 커질수록 채점이 운에 가까워진다.** §8.1이 지적한 "봇 표가 무작위다"가 정원에 비례해 심해진다. 정원 8인 방에 사람이 2명이면 **표 8장 중 6장이 무작위**다. 사람이 많이 모이면 문제가 없지만, 정원만 크게 잡고 둘이 들어간 방은 5인 방보다 더 나빠진다. §8.1의 두 선택지(봇이 근거를 갖고 투표 / 봇 표를 점수에서 제외) 중 어느 쪽을 고르든, 정원 상한 8을 기준으로 계산한다.
