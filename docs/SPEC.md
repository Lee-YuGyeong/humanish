...
**→ 그 뒤집기가 2026-08-08에 다시 되돌아왔다 (§18.5). 봇은 주제에 100% 답한다.** 그래서 이 문단의 원래 서술("봇도 반드시 답한다")이 지금 코드와 같다.
...
**감싸는 것은 `/main`·`/room/[code]`·`/world` 다** — 게임 그 자체다. `/world` 는 2026-08-08에 들어왔다: `/world?code=ABCD` 가 사람들이 실제로 복사해 보내는 주소라, 열어 두면 로그인 안 한 사람이 티켓 발급이 401 로 끊기는 자리까지 갔다가 에러 화면을 본다.

**`/intro` 는 첫 화면이라 감싸지 않고, `/lab`·`/admin` 은 개발용 점검 화면이라 게임 계정과 무관하게 돈다** (CLAUDE.md 라우트).

**돌아갈 주소는 쿼리까지 통째로 들고 간다.** `/login?next=%2Fworld%3Fcode%3DABCD` → 구글 → 콜백 → 그 주소. 경로만 담으면 `?code=` 가 날아가서, 초대 링크를 받은 사람이 로그인 뒤에 자기가 초대받은 방이 아니라 로비로 떨어진다.
...
- **봇도 답을 거른다.** 봇마다 약 15% 확률로 그 페이즈의 답변을 만들지 않는다. 빈칸이 양쪽에서 생기므로 "답 없는 자리 = 사람"이 깨진다. §5.3이 "봇 답변이 하나라도 비면 그 자리가 사람으로 확정된다"고 적은 건 **비는 일이 사람 쪽에서만 생길 때** 맞는 말이었다.
  - **→ 2026-08-08 되돌렸다 (사용자 결정). 봇은 주제에 100% 답한다** (`BOT_SILENCE_CHANCE = 0`). 거른 봇도 사람이 말을 걸면 대꾸는 했기 때문에 — 월드에서는 그래야 45초가 통째로 조용해지지 않는다 — 화면에 **"주제는 씹고 옆사람 말에는 꼬박꼬박 답하는 자리"** 가 남았다. 사람의 패스는 창을 통째로 흘려보내는 모양이라 둘이 겹치지 않는다. **빈칸을 만들려던 장치가 다른 모양의 표식이 됐다.**
  - **되살리려면 확률만 올려서는 안 된다.** 거른 창은 **대꾸까지 막아야** 사람의 패스와 같은 모양이 된다 (`primeForTopic` 의 `speak=false` 가 `topicDue` 를 0 이 아니라 창 끝으로 두는 쪽). 아래 세 줄은 그 조건에서만 성립한다.
...
| §5.3 | 봇은 반드시 답한다 | 약 15% 확률로 거른다 (§18.5) → **2026-08-08 되돌림. 주제에는 100% 답한다** |
...
| 답을 안 낸 자리는 사람 확정 | 봇도 약 15% 확률로 거른다 (§18.5) → **2026-08-08 되돌림.** 대신 최후변론 15% 침묵만 남았다 |
