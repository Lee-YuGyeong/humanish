...
| **15-2** | ~~익명 인증 도입 여부~~ **결정: 붙인다.** 익명 인증을 기반으로 깔고 그 위에 구글을 **연결**한다. 기존 방 테이블의 RLS는 손대지 않는다. 근거는 §15-2-결정 | — |
...
### 15-2-결정 — 익명 인증을 깔고 그 위에 구글을 연결한다

**결정: 익명 인증을 붙인다. 로그인 화면은 없다. 구글은 그 위에 잇는다.** (2026-08-01)

전적·랭킹·친구 목록을 만들기로 하면서 계정이 필요해졌다. 그런데 이 게임은 익명이 전제다 — 첫 화면에 로그인 벽을 세우면 3분 놀러 온 사람이 거기서 나간다. 두 요구가 부딪히므로 **순서로 푼다.**

| | 무엇 | 사용자가 하는 일 |
|---|---|---|
| 1 | 들어오면 `signInAnonymously()` | 없음 — 조용히 계정이 생긴다 |
| 2 | 게임 끝 화면에서 `linkIdentity({provider:'google'})` | 클릭 한 번 |

**이어붙이므로 `user_id`가 안 바뀐다.** 익명일 때 쌓인 것이 그대로 따라온다. 반대 순서(구글 먼저)는 되돌릴 수 없다.

**자체 로그인(이메일+비밀번호)을 쓰지 않는 이유**: 비밀번호 찾기 메일·가입 확인 메일·SMTP 설정·문의 대응이 영구히 따라온다. 그리고 2단계가 "한 번 클릭"이 아니라 "메일함 왕복"이 되어 흐름이 죽는다. **제공자를 나중에 더하는 건 쉽고, 비밀번호를 나중에 걷어내는 건 어렵다.**
...
전적·랭킹·친구는 이 결정 위에 얹는 다음 단계다. 랭킹을 만들 때 **부정 유인**을 같이 막아야 한다 — 정원 5인 방을 혼자 만들면 봇이 4명이고, 혼자면 스파이도 안 뽑힌다(§8). 봇만 맞히며 점수를 쌓을 수 있다. 전적 기록 조건을 **사람 2명 이상인 방**으로 걸어두면 막힌다. 나중에 붙이면 이미 쌓인 점수를 어떻게 할지 곤란해진다.

#### 운영에서 해야 하는 것

- Supabase 대시보드 → Auth → **Anonymous sign-ins** 켜기
- Auth → Providers → **Google** 켜기 (클라이언트 ID·시크릿)
- Auth → **Manual linking** 켜기 — 없으면 `linkIdentity`가 안 된다
...
- 익명 계정은 자동으로 안 지워진다. 오래된 것을 주기적으로 지우는 작업이 나중에 필요하다.
- 익명 로그인에는 IP당 한도가 있다. 한 곳에서 여럿이 들어오면 걸릴 수 있다.
