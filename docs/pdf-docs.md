...
| 게임 소개 및 설명 | `docs/game-intro.html` | `docs/game-intro/` (스크린샷 15장) | `게임 소개 및 설명 문서 v2.pdf` |
| AI 활용 기술 문서 | `docs/ai-usage.html` | 없음 | `AI 활용 기술 문서 v2.pdf` |

**PDF 는 저장소에 커밋하지 않는다** (`.gitignore` 의 `/*.pdf`). 산출물이지 소스가 아니라서,
소스와 어긋난 PDF 가 이력에 남는 쪽이 더 나쁘다. 이름 뒤 `v2` 는 소스가 없던 원본과
구분하려고 붙인 것이고, 아래 명령의 출력 이름과 같아야 한다 — **한쪽만 고치면
심사자가 옛 파일을 본다.**
