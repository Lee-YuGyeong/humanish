...
- **I1. `is_bot`을 클라이언트로 내보내지 않는다.** 클라이언트는 `players`가 아니라 `public_players` 뷰를 읽는다. 이게 새면 게임이 즉시 끝난다. **뷰에 컬럼을 더할 때마다 "이걸로 봇을 골라낼 수 있나"를 먼저 묻는다** (SPEC §7.2).
...
- **I5. 조기 종료 인원은 `is_bot = false`만 센다.** 봇을 포함해 세면 모든 페이즈가 시작 즉시 끝난다. **인원을 세지 않는 조기 종료도 같다** — target 대상이 봇이면 조기 종료하지 않는다 (SPEC §5.3).
