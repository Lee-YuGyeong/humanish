...
최대 10자리(사람 8 + AI 2)인 채팅방에서 누가 AI인지 찾는 웹 게임. 방을 만들 때 **총 자리**를 정한다 (3~10 중 **6 제외**, 기본 5).
**AI가 몇인지는 시작할 때의 사람 수로 정해지고 공개하되, 어느 자리인지는 끝까지 숨긴다** (SPEC §15-3, §18.1).
...
**아직 코드에 반영되지 않았다** — 규칙과 코드가 다르면 규칙이 맞고, 손대는 순서는 §18.7에 있다.
...
- **정원은 방마다 다르다 (SPEC §17.6, §18.1).** 좌석 배열·`PlayerGrid`·`Array(5)`처럼 **숫자를 하드코딩하지 않는다.** DB에서는 `room_capacity()`를, 코드에서는 `room.capacity`를 쓴다. `rooms`를 select할 때 그 컬럼을 빠뜨리면 그리드가 0칸이 된다.
  **수가 둘이고 뜻이 다르다 (SPEC §18.1).** `capacity`는 **총 자리의 상한**(3~10 중 **6 제외**, 기본 5)이고, 그 판의 **실제 자리 수는 `seat_count`**(3~10)다. `seat_count`는 시작 시점의 사람 수로 인원표를 읽어 정해지고 그전에는 `null`이다 — 총 자리 10인 방이 사람 4명으로 시작하면 좌석은 5칸이고 AI는 1대다. **좌석 그리드는 `seat_count`로 그린다.** 6을 뺀 이유는 그 값이 5와 같은 판이 되기 때문이다.
