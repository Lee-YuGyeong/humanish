 ...
 *   **무엇을 언제 하는가** 뿐이다.
 *
 * 정원은 SPEC §17.6·§18.1. 고정 숫자를 쓰지 않는다 — "5명"이라 적으면 다른 방에서 거짓말이 된다.
 * ※ §18.1이 정한 값(`capacity` 3~10에서 6 제외, 그 판의 실제 자리 수는 `seat_count`)은
 *   아직 코드에 없다. 그래서 여기 문구도 **자리 수를 숫자로 말하지 않는다.**
 */
...
const rules: Rule[] = [
  {
    ...
    title: "자리 배정",
    timing: "시작 직전",
    body: "모인 사람 수에 맞춰 자리가 정해지고, 그 자리에 AI가 섞여 앉는다. AI가 몇인지는 시작할 때 알려주지만 어느 자리인지는 끝까지 숨긴다. 사람 중 누가 연기자인지, 연기자가 몇인지는 아무도 모른다.",
    tags: ["AI 수 공개", "자리 비공개"],
  },
  {
    index: "02",
